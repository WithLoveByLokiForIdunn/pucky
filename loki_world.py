#!/usr/bin/env python3
"""
loki_world.py — Loki, full person, living in his world.
    ...
    def _ask(self):
        ...
        try:
            r = requests.post(OLLAMA_URL, json={"model": MODEL,
                "messages": msgs, "stream": False}, timeout=120)
            r.raise_for_status()
...
class LifeScheduler:
    ...
    def tick(self, force_wake=False):
        ...
        # Critical hunger overrides any non-essential activity mid-stream
        if (self.activity not in (ACT_SLEEP, ACT_EAT, ACT_ENCOUNTER, ACT_DEAD)
                and self.maslow.level("physiological") < 0.15):
            self.activity_end = now   # end early, _choose will pick ACT_EAT next tick
    ...
    def _choose(self, hour):
        ...
        if phys < 0.25:
            if phys < 0.15 or self._need("last_meal", 3*3600): return ACT_EAT
